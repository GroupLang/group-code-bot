...
async def handle_github_issue_link(message: Dict[str, Any]) -> None:
    ...
    async with AgentMarketClient() as client:
        try:
            ...
            from services.bot.messages import Emoji
            
            message_text = (
                f"{Emoji.SUCCESS} Created instance `{instance_id}` from GitHub issue #{issue_number}\n\n"
                f"{Emoji.MEMO} *Issue Details:*\n"
                f"*Title:* {issue['title']}\n"
            )
            ...
            if issue.get('body'):
                ...
                message_text += f"\n{Emoji.CHAT} *Description:*\n{body}\n"
                
            if issue.get('reward_amount'):
                message_text += f"\n{Emoji.ROCKET} *Reward Amount:* ${issue['reward_amount']:.2f}"
